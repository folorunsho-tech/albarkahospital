...
router.get("/drugs/:id", async (req, res) => {
  try {
    const found = await prisma.drugs.findUnique({
      where: {
        id: req.params.id,
      },
    });
...
router.post("/drugs/many", async (req, res) => {
  try {
...
router.post("/drugs/edit/:id", async (req, res) => {
  try {
...
router.post("/fees/many", async (req, res) => {
  try {
